    ...
    with open(file) as f:
        ...
        for line in lines:
            if not line.startswith("// function"):
                print(line.rstrip())
                continue
            function = line.strip().split()[2]
            sym = symbols[function]
            print(DOCUMENTATION_COMMENT.format(
                name=sym.name, vram=sym.vram, vrom=sym.vrom, size=sym.size
            ))
...
def main():
    ...
    parser.add_argument("cfile", help="C file to read.")
    ...
    process_file(args.cfile, symbols)
