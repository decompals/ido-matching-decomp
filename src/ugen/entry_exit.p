...
begin
    ...
    if argA then begin
        if stack_limit_bn = 0 then begin
            ...
            spBB := xr8;
            demit_ra(zlw, xr8, stack_limit_bn, 0);
        end;
    end;
    if (arg3 <> 0) and arg9 then begin
        if isa = ISA_MIPS2 then begin
            if reversed_stack then begin
                demit_rri(ztltu, spBB, xr29, 9);
            end else begin
                demit_rri(ztltu, xr29, spBB, 9);
            end;
        end else begin
            spBC := gen_label_id();
            if reversed_stack then begin
                demit_rrll(zbleu, xr29, spBB, spBC);
            end else begin
                demit_rrll(zbgeu, xr29, spBB, spBC);
            end;
            demit_i(zbreak, 9);
            ddefine_label(spBC);
        end;
...
begin
    ...
    if init_dynmem and (arg2 <> 0) then begin
        ...
        emit_rrll(zbne, sp2A, xr29, sp2C);
        free_reg(sp2B);
        free_reg(sp2A);
    end;
    gen_reg_save(arg2, arg3, arg5, arg4);
    if home_vararg_reg then begin
        emit_rob(zsd, xr4, arg2, xr29, 0);
        emit_rob(zsd, xr6, arg2 + 8, xr29, 0);
    end;
end;
...
begin
    ...
    if xr30 in saved_regs then begin
        if reversed_stack then begin
            if basicint = 0 then begin
                var_s1 := var_s1 + 4;
    ...
    if pic_level > 0 then begin
        if xr28 in saved_regs then begin
            if reversed_stack then begin
                var_s1 := var_s1 + 4;
            ...
            sp48 := bitor(sp48, lshift(1, ord(xr28)));
    ...
    i := 0;
    var_s0 := registers(ord(xr16) + n_saved_regs - 1);
    ...
    if frame_pointer <> xr29 then begin
        ...
        if not arg4 then begin
            emit_rr(zmove, xr29, frame_pointer);
        end;
    end;
    i := 0;
    ...
    if (pic_level > 0) and (xr28 in saved_regs) then begin
        if reversed_stack then begin
            var_s1 := var_s1 - 4;
        end else begin
            var_s1 := var_s1 + 4;
    ...
    if arg6 <> -1 then begin
        sp48 := bitor(sp48, lshift(1, ord(xr31)));
        if basicint = 0 then begin
            emit_rob(zlw, xr31, arg0 + arg6, xr29, 0);
        end else begin
            emit_rob(zld, xr31, arg0 + arg6, xr29, 0);
        end;
        if xr30 in saved_regs then begin
            if reversed_stack then begin
                if basicint = 0 then begin
                    emit_rob(zlw, xr30, -var_s1, frame_pointer, 0);
                end else begin
                    emit_rob(zld, xr30, -var_s1, frame_pointer, 0);
                end;
            end else begin
                if basicint = 0 then begin
                    emit_rob(zlw, xr30, var_s1, frame_pointer, 0);
        ...
        if xr31 in saved_regs then begin
            if xr30 in saved_regs then begin
                if basicint = 0 then begin
                    emit_rob(zlw, xr31, -var_s1 + 4, frame_pointer, 0);
                    emit_rob(zlw, xr30, -var_s1, frame_pointer, 0);
                end else begin
                    emit_rob(zld, xr31, -var_s1 + 8, frame_pointer, 0);
                    emit_rob(zld, xr30, -var_s1, frame_pointer, 0);
                end;
            end else begin
                if basicint = 0 then begin
                    emit_rob(zlw, xr31, -var_s1, frame_pointer, 0);
                end else begin
                    emit_rob(zld, xr31, -var_s1, frame_pointer, 0);
                end;
            end;
        end else if xr30 in saved_regs then begin
            if basicint = 0 then begin
                emit_rob(zlw, xr30, -var_s1, frame_pointer, 0);
            end else begin
                emit_rob(zld, xr30, -var_s1, frame_pointer, 0);
...
begin
    ...
    if xr31 in saved_regs then begin
        if reversed_stack then begin
            if basicint = 0 then begin
                var_s1 := var_s1 + 4;
                emit_rob(zsw, xr31, -var_s1, xr29, 0);
            end else begin
                var_s1 := var_s1 + 8;
                emit_rob(zsd, xr31, -var_s1, xr29, 0);
            end;
        end else begin
            if basicint = 0 then begin
                var_s1 := var_s1 - 4;
    ...
    if xr30 in saved_regs then begin
        if reversed_stack then begin
            if basicint = 0 then begin
                var_s1 := var_s1 + 4;
                emit_rob(zsw, xr30, -var_s1, xr29, 0);
            end else begin
                var_s1 := var_s1 + 8;
                emit_rob(zsd, xr30, -var_s1, xr29, 0);
            end;
        end else begin
            if basicint = 0 then begin
                var_s1 := var_s1 - 4;
    ...
    while i <> n_saved_regs do begin
        if var_s0 in saved_regs then begin
            if reversed_stack then begin
                if basicint = 0 then begin
                    var_s1 := var_s1 + 4;
                    emit_rob(zsw, var_s0, -var_s1, xr29, 0);
                end else begin
                    var_s1 := var_s1 + 8;
                    emit_rob(zsd, var_s0, -var_s1, xr29, 0);
                end;
            end else begin
                if basicint = 0 then begin
                    var_s1 := var_s1 - 4;
    ...
    while i <> n_saved_fp_regs do begin
        if var_s0 in saved_regs then begin
            if reversed_stack then begin
                var_s1 := var_s1 + 8;
                emit_rob(fs_d, var_s0, -var_s1, xr29, 0);
            end else begin
                var_s1 := var_s1 - 8;
                emit_rob(fs_d, var_s0, var_s1, xr29, 0);
            end;
    ...
    if frame_pointer <> xr29 then begin
        emit_rr(zmove, frame_pointer, xr29);
    end;
end;
